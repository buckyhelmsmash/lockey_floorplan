...
const APP_NAMESPACE = '__lockey_floorplan__';
...
export const loadDiagram = (data) => {
  const newState = {
    selected: null,
    shapes: JSON.parse(data),
  };

  useShapes.set(newState);
};

export const generateJSON = () => JSON.stringify(useShapes.get().shapes);
...
export const transformCircleShape = (node, id, event) => {
  ...
  setState((state) => {
    ...
    if (shape) {
      ...
      shape.radius = clamp((node.width() * scaleX) / 2, LIMITS.CIRCLE.MIN, LIMITS.CIRCLE.MAX);
    }
  });
};
